...
class Detector3DTemplate(nn.Module):
    ...
    def generate_recall_record(box_preds, recall_dict, batch_index, data_dict=None, thresh_list=None):
        ...
        if recall_dict.__len__() == 0:
            recall_dict = {'gt': 0}
            recall_dict.update({'dt': 0})
            for cur_thresh in thresh_list:
                recall_dict['roi_%s' % (str(cur_thresh))] = 0
                recall_dict['rcnn_%s' % (str(cur_thresh))] = 0
                # precision
                recall_dict['roi_P_%s' % (str(cur_thresh))] = 0
                recall_dict['rcnn_P_%s' % (str(cur_thresh))] = 0
        ...
        if cur_gt.shape[0] > 0:
            ...
            for cur_thresh in thresh_list:
                ...
                # for precision
                if iou3d_rcnn.shape[1] == 0:
                    recall_dict['rcnn_P_%s' % str(cur_thresh)] += 0
                else:
                    rcnn_recalled = (iou3d_rcnn.max(dim=1)[0] > cur_thresh).sum().item()
                    recall_dict['rcnn_P_%s' % str(cur_thresh)] += rcnn_recalled
                if rois is not None:
                    roi_recalled = (iou3d_roi.max(dim=1)[0] > cur_thresh).sum().item()
                    recall_dict['roi_P_%s' % str(cur_thresh)] += roi_recalled

            recall_dict['gt'] += cur_gt.shape[0]
            recall_dict['dt'] += box_preds.shape[0]
        else:
