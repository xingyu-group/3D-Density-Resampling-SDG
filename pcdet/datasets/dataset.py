...
class DatasetTemplate(torch_data.Dataset):
    def __init__(self, dataset_cfg=None, class_names=None, training=True, root_path=None, logger=None):
        ...
        if self.oss_path is not None:
            ...
            # Augmentations during testing (for TTT)
            self.data_augmentor_test = DataAugmentor(
                self.root_path, self.dataset_cfg.DATA_AUGMENTOR_TESTING, self.class_names, logger=self.logger, oss_flag=False
            ) if (not self.training) and (self.dataset_cfg.get('DATA_AUGMENTOR_TESTING', False)) else None    
    ...
    def prepare_data(self, data_dict):
        ...
        if (not self.training) and self.dataset_cfg.get('DATA_AUGMENTOR_TESTING', False):
            data_calib = data_dict['calib'] if data_dict.get('calib', False) else None
            data_dict = self.data_augmentor_test.forward(
                data_dict={
                    **data_dict,
                }
            )
            if data_calib is not None:
                data_dict['calib'] = data_calib
    ...
    def collate_batch(batch_list, _unused=False):
        ...
        for key, val in data_dict.items():
            try:
                ...
                elif key in ['points', 'voxel_coords', 'points_org']:
                    coors = []
