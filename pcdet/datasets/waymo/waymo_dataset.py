...
class WaymoDataset(DatasetTemplate):
    ...
    def __init__(self, dataset_cfg, class_names, training=True, root_path=None, logger=None):
        ...
        self.get_beam = dataset_cfg.get('READ_BEAM_LABEL', False)
    ...
    def get_lidar(self, sequence_name, sample_idx, tail_txt='', get_beam=False):
        data_path_custom = self.root_path / (self.dataset_cfg.PROCESSED_DATA_TAG + tail_txt) 
        if self.oss_path is None:
            #lidar_file = self.data_path / sequence_name / ('%04d.npy' % sample_idx) #
            lidar_file = os.path.join(data_path_custom, sequence_name, ('%04d.npy' % sample_idx))
        ...
        if get_beam:
            beam_labels_file = self.data_path / 'beam' /sequence_name / ('%04d.beam_labels.npy' % sample_idx)
    ...
    def __getitem__(self, index):
        ...
        if self.use_shared_memory and index < self.shared_memory_file_limit:
            ...
            if self.dataset_cfg.get('AUG_LIDAR_READING', False) and np.random.rand(1).item() <= float(self.dataset_cfg.AUG_LIDAR_READING.POB):
                points = self.get_lidar(sequence_name, sample_idx, tail_txt=self.dataset_cfg.AUG_LIDAR_READING.TAIL_TXT)
            else:
                if self.get_beam:
                    points, beam_labels = self.get_lidar(sequence_name, sample_idx, get_beam=self.get_beam)
                else:
                    points = self.get_lidar(sequence_name, sample_idx)
        ...
        if self.get_beam:
            input_dict.update({'beam_labels': beam_labels})
        ...
        data_dict['dataset_domain'] = 'waymo'
        return data_dict
