...
class NuScenesDataset(DatasetTemplate):
    ...
    def get_sweep(self, sweep_info, head_txt=''):
        def remove_ego_points(points, center_radius=1.0):
            mask = ~((np.abs(points[:, 0]) < center_radius) & (np.abs(points[:, 1]) < center_radius))
            return points[mask]
        
        lidar_path_custom = head_txt + sweep_info['lidar_path']

        if self.oss_path is None:
            lidar_path = self.root_path / lidar_path_custom
            points_sweep = np.fromfile(str(lidar_path), dtype=np.float32, count=-1).reshape([-1, 5])[:, :4]
        else:
            lidar_path = os.path.join(self.oss_path, lidar_path_custom)
    ...
    def get_lidar_with_sweeps(self, index, max_sweeps=1, head_txt=''):
        info = self.infos[index]

        lidar_path_custom = head_txt + info['lidar_path']
        ...
        for k in np.random.choice(len(info['sweeps']), max_sweeps - 1, replace=False):
            points_sweep, times_sweep = self.get_sweep(info['sweeps'][k], head_txt)
    ...
    def __getitem__(self, index):
        ...
        if self.dataset_cfg.get('AUG_LIDAR_READING', False) and np.random.rand(1).item() <= float(self.dataset_cfg.AUG_LIDAR_READING.POB):
            points = self.get_lidar_with_sweeps(index, max_sweeps=self.dataset_cfg.MAX_SWEEPS, head_txt=self.dataset_cfg.AUG_LIDAR_READING.HEAD_TXT)
        else:
        ...
        data_dict['dataset_domain'] = 'nuscenes'
    ...
    def evaluation(self, det_annos, class_names, **kwargs):
        if kwargs['eval_metric'] == 'kitti':
            ...
            # print(eval_det_annos[0]['frame_id'])
            # print(eval_gt_annos[0])
            # print(eval_gt_annos[0]['frame_id'])
