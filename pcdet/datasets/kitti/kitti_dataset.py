...
class KittiDataset(DatasetTemplate):
    ...
    def get_lidar(self, idx, tail_txt=''):
        if self.oss_path is None:
            lidar_file = self.root_split_path / ('velodyne%s' % tail_txt) / ('%s%s.bin' % (idx, tail_txt))
            assert lidar_file.exists()
            points = np.fromfile(str(lidar_file), dtype=np.float32).reshape(-1, 4)
        else:
            lidar_file = os.path.join(self.root_split_path, ('velodyne%s' % tail_txt), ('%s%s.bin' % (idx, tail_txt)))
    ...
    def __getitem__(self, index):
        ...
        if "points" in get_item_list:
            ...
            if self.dataset_cfg.get('AUG_LIDAR_READING', False) and np.random.rand(1).item() <= float(self.dataset_cfg.AUG_LIDAR_READING.POB):
                points = self.get_lidar(sample_idx, tail_txt=self.dataset_cfg.AUG_LIDAR_READING.TAIL_TXT)
            else:
                points = self.get_lidar(sample_idx)
        ...
        data_dict['dataset_domain'] = 'kitti'
        return data_dict
