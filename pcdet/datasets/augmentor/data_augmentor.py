...
from . import multi_aug_utils
import random
import time

from ...ops.roiaware_pool3d import roiaware_pool3d_utils
from ...utils import downsample_utils
from .VField_aug.VField_aug import VField
...
class DataAugmentor(object):
    def __init__(self, root_path, augmentor_configs, class_names, logger=None, oss_flag=False):
        ...
        for cur_cfg in aug_config_list:
            if 'vf_adversarial_pert' == cur_cfg.NAME:
                self.vf_transformer = VField(classes_interest=cur_cfg.CLASSES, VF_file=cur_cfg.VF_FILE)
    ...
    def random_object_scaling(self, data_dict=None, config=None):
        ...
        for obj_class in list(config['SCALE_UNIFORM_NOISE'].keys()):
            mask_cls_obj = data_dict['gt_names'] == obj_class
            points, gt_boxes = augmentor_utils.scale_pre_object(
                data_dict['gt_boxes'][mask_cls_obj], points,
                # gt_boxes_mask=data_dict['gt_boxes_mask'],
                scale_perturb=config['SCALE_UNIFORM_NOISE'][obj_class]
            )
            new_gt_boxes.append(gt_boxes)
            new_gt_names += [obj_class] * mask_cls_obj.sum()

        data_dict['gt_boxes'] = np.concatenate(new_gt_boxes, axis=0)
        data_dict['gt_names'] = np.array(new_gt_names)
        data_dict['points'] = points
        return data_dict
    ...
    def normalize_object_size_3objs_without_intensity(self, data_dict=None, config=None):
        if data_dict is None:
            return partial(self.normalize_object_size_3objs_without_intensity, config=config)
        
        assert isinstance(config['SIZE_RES'], dict), 'For multi-class object SN, multi-SIZE_RES should be offered.'

        if data_dict['gt_boxes'].shape[1]>7:
            dimension_append = data_dict['gt_boxes'][:,7:] 

        gt_boxes, points = data_dict['gt_boxes'][:, :7], data_dict['points']
        for class_obj in list(config.SIZE_RES.keys()):
            gt_boxes_mask_cur_class = data_dict['gt_names'] == class_obj
            points, gt_boxes = augmentor_utils.normalize_object_size(
                gt_boxes, points, gt_boxes_mask_cur_class, config['SIZE_RES'][class_obj]
            )
        
        if data_dict['gt_boxes'].shape[1]>7:
            gt_boxes = np.hstack((gt_boxes, dimension_append.reshape(gt_boxes.shape[0], -1)))
            assert gt_boxes.shape == data_dict['gt_boxes'].shape
    ...
    def multi_aug_common_corruption(self, data_dict=None, config=None):
        if data_dict is None:
            return partial(self.multi_aug_common_corruption, config=config)
        
        aug_list = config['AUG_LIST']
        num_selected_aug = int(config['NUM_SELECTED_AUG'])
        severity_list = config['SEVERITY_LIST']

        augs_selected = random.sample(aug_list, num_selected_aug)

        if config.get('ORG_SAVE', False):
            data_dict['points_org'] = copy.deepcopy(data_dict['points'])
        
        if not data_dict.get('layer_num_sim', False):
            data_dict['layer_num_sim'] = config.LAYER_NUM

        Layer_num = data_dict['layer_num_sim']

        for aug in augs_selected:
            # severity = int(random.choice(severity_list)) if Layer_num <= config.LAYER_NUM else 5
            severity = int(random.choice(severity_list))
            data_dict['points'] = multi_aug_utils.__all__[aug](
                pointcloud=data_dict['points'], 
                severity=severity, 
                bin_num=Layer_num, 
                shift_origin=config.SHIFT_COOR,
                random_choice_layer_with_step= config.get('RANDOM_SELECTION', False)
            )
            if aug == 'layer_del_scene':
                data_dict['layer_num_sim'] = int(data_dict['layer_num_sim'] / severity)
            elif aug == 'layer_interp_scene':
                data_dict['layer_num_sim'] *= 2
            else:
                data_dict['layer_num_sim'] = data_dict['layer_num_sim']
    ...
    def vf_adversarial_pert(self, data_dict=None, config=None):
        if data_dict is None:
            return partial(self.vf_adversarial_pert, config=config)

        boxes_list = []
        names_list = []
        for cls_obj in config.CLASSES_AFFECTED:
            index_cls = np.where(data_dict['gt_names'] == cls_obj)[0]
            if index_cls.shape[0] > 0:
                num_obj = min(int(config.NUM_OBJ_PER_CLASS), index_cls.shape[0])
                index_selected = np.random.choice(index_cls, num_obj, replace=False)
                boxes_list.append(data_dict['gt_boxes'][index_selected].reshape(index_selected.shape[0], -1))
                names_list.append(data_dict['gt_names'][index_selected])
        
        boxes_selected = np.concatenate(boxes_list, axis=0)
        names_selected = np.concatenate(names_list, axis=0)
        
        data_dict['points'] = self.vf_transformer.perturbation(data_dict['points'], boxes_selected, names_selected)
