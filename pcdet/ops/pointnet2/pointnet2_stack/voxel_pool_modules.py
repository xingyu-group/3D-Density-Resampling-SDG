...
class NeighborVoxelLinearPoolModuleMSG(nn.Module):
    ## except mlps, only maxpool or avepool features          
    def __init__(self, *, query_ranges: List[List[int]], radii: List[float], 
        nsamples: List[int], use_xyz: bool = True, pool_method='ave_pool'):
        """
        Args:
            query_ranges: list of int, list of neighbor ranges to group with
            nsamples: list of int, number of samples in each ball query
        ...
        assert len(query_ranges) == len(nsamples)
