...
from pcdet.datasets.augmentor import multi_aug_utils
...
def eval_one_epoch(cfg, model, dataloader, epoch_id, logger, dist_test=False, save_to_file=False, result_dir=None):
    ...
    for i, batch_dict in enumerate(dataloader):
        ...
        # print(list(batch_dict.keys())) # voxelrcnn: ['db_flag', 'frame_id', 'calib', 'gt_boxes', 'points', 'use_lead_xyz', 'image_shape', 'dataset_domain', 'batch_size']
        with torch.no_grad():
...
def eval_one_epoch_bin_acc_parallel(cfg, model, show_db, dataloader_s1, dataloader_s2, epoch_id, logger, dist_test=False, save_to_file=False, result_dir=None):
    result_dir.mkdir(parents=True, exist_ok=True)
    ...
    if show_db == 1:
        ...
        for i, batch_1 in enumerate(dataloader_s1):
            ...
            with torch.no_grad():
                pred_dicts = model(batch_1, batch_2)
            disp_dict = {}
            ...
            #     output_path=final_output_dir if save_to_file else None
            # )
            det_annos += [pred_dicts]
            if cfg.LOCAL_RANK == 0:
                progress_bar.set_postfix(disp_dict)
                progress_bar.update()
    elif show_db == 2:
        raise NotImplementedError('For evaluation on the domain discriminator, only db1 (the larger dataset) is allowed to show.')
    ...
    result_dict = {}
    for key in list(det_annos[0].keys()):
        result_dict[key] = 0

    for anno in det_annos:
        for key in list(anno.keys()):
            result_dict[key] += anno[key]
    
    result_str = '\n'
    for key in list(result_dict.keys()): 
        result_str += key + (' accuracy: %.4f\n' % (result_dict[key]/len(det_annos)))  
