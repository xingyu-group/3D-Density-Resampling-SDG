...
def train_one_epoch_multi_db(model, optimizer, train_loader_1, train_loader_2, model_func, lr_scheduler, accumulated_iter, optim_cfg,
    ...
    for cur_it in range(total_it_each_epoch):
        ...
        ret_dict_s1, tb_dict_s1, disp_dict_s1 = model(batch_1)
        # loss_s1 = ret_dict_s1['loss'].mean()
        ...
        ret_dict_s2, tb_dict_s2, disp_dict_s2 = model(batch_2)
        # loss_s2 = ret_dict_s2['loss'].mean()
        if hasattr(model, 'update_global_step'):
            model.update_global_step()
        else:
            model.module.update_global_step()
        ...
        loss = 0
        loss_disp_dict = {}
        for loss_item in list(ret_dict_s1['loss'].keys()):
            loss += ret_dict_s1['loss'][loss_item].mean()
            loss += ret_dict_s2['loss'][loss_item].mean()
            loss_disp_dict[loss_item+'_1'] = ret_dict_s1['loss'][loss_item].item()
            loss_disp_dict[loss_item+'_2'] = ret_dict_s2['loss'][loss_item].item()
        ...
        accumulated_iter += 1
        # disp_dict_s1.update({'loss1': loss_s1.item(),'loss2': loss_s2.item(), 'lr': cur_lr})
